 ...
 */
public class LocalDateTimeTableCell<R> extends BaseCell<R, LocalDateTime> {

    private DatePicker datePicker;
    private LocalDateTimeColumn<R> column;

    public LocalDateTimeTableCell(LocalDateTimeColumn<R> column) {
        super(column);
        this.column = column;
    }
    ...
    protected void setValueToEditor(LocalDateTime value) {
         ...
         * not committed to datePicker until user presses ENTER or the
         * datepicker lost the focus. If user change the date with typing and
         * then presses ESC to cancel, the text field is not automatically
         * updated because datePicker doesn't detect any value change. So, let's
         * set the textfield manually
         */
        LocalDate date = value == null ? LocalDate.now() : value.toLocalDate();
        datePicker.getEditor().setText(datePicker.getConverter().toString(date));
    ...
    protected LocalDateTime getValueFromEditor() {
        ...
        LocalDateTime value = datePicker.getValue().atStartOfDay();
        try {
            value = c.fromString(dateText).atStartOfDay();
        ...
        datePicker.addEventFilter(KeyEvent.KEY_PRESSED, new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent t) {
                if (t.getCode() == KeyCode.ENTER) {
                     ...
                     */
                    LocalDateTimeTableCell.this.fireEvent(t);
                }
            }
        });
    }
